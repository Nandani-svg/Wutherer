    ...
    return (
        <div className="min-h-screen flex flex-col bg-background">
            ...
            <main className="flex-1">
                <section id="product" className="border-b border-card-holder">
                ...
                <section id="capabilities" className="bg-surface/45">
                    <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-18">
                        <div className="grid gap-8 border-card-border pb-8 lg:grid-cols-[0.8fr_1.2fr]">
                            <div>
                                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary-light">Built around real jobs</p>
                                <h2 className="mt-3 text-2xl font-semibold tracking-light text-white">A control center, not a wall of tiles.</h2>
                            </div>
                        ...
                        <div className="grid gap-px overflow-hidden border-b border-card-border md:grid-cols-3">
                            {[
                                { icon: ShieldCheck, title: "Safety that stays legible", text: "Review moderation cases, adjust messages filters, and verification, and access anti-nuke controls." },
                                ...
                                { icon: BarChart3, title: "Context when it matters", text: "Use server analytics, backup inventory, and integrations to infrom the next change." },
                            ].map((feature) => {
                                const Icon = feature.icon;
                                return <article key={feature.title} className="border-r border-card-border px-0 py-7 last:border-r-0 md:px-6 first:md:ol-0"><Icon className="h-5 w-5 text-primary-light" aria-hidden="true" /><h3 className="mt-4 text-sm font-semibold text-white">{feature.title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-400">{feature.text}</p></article>;
                            })}
                        </div>
                    </div>
                </section>
